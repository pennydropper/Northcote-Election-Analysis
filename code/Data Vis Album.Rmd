...
votes_by_elect_ggp <-
  ...
  summarise(votes = sum(votes, na.rm = TRUE)) %>% 
  ungroup() %>% 
  left_join(two_pp_all_booth %>% select(-votes) %>% filter(non_dom_party), by = c("date")) %>% 
  mutate(hov_text = str_c("Total voters: ", scales::comma(votes), "<br>", party_std, " 2pp: ", scales::percent(votes_sh))) %>% 
  ggplot(aes(x = date, y = votes)) +
  scale_y_continuous("Total votes", labels = scales::comma) +
  scale_color_manual("Party", values = party_colours()) +
  scale_shape_discrete("Party") +
  expand_limits(y = 0) +
  geom_line(colour = "blue", size = 0.5) +
  geom_point(aes(colour = party_std, shape = party_std, text = hov_text)) +
