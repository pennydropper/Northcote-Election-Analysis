...
votes_by_phys_booth <-
  ...
  group_by(year) %>% 
  mutate(votes_sh_scale = scales::rescale(votes_sh)) %>% 
  ungroup %>% 
  # print()
  glimpse
...
pal <- colorNumeric(
  palette = "RdYlGn",
  domain = votes_by_phys_booth$votes_sh_scale)
...
leaflet(options = leafletOptions(minZoom = 12,
  ...
  addPolygons(data = nrthct_plgon,
    ...
    addCircleMarkers(data = votes_by_phys_booth %>% filter(year == "2018"),
                   radius = ~votes / 100,
                   popup = ~hov_text,
                   label = ~booth,
                   lat = ~lat,
                   ...
                   fill = TRUE) %>% 
  addLegend(title = "2pp vs ALP", pal = pal, values = votes_by_phys_booth$votes_sh,
            labels = palette(),
